...
class TreeQN(nn.Module):
    def __init__(self, input_shape, embedding_dim, num_actions, tree_depth, td_lambda):
        ...
        self.decoder = CNN_Decoder(embedding_dim, self.flat_conv_dim, self.h1, self.h2)
        ...
        self.value_fun = nn.Linear(embedding_dim, 1) #literally its just this except with the w_scale
    ...
    def tree_plan(self, tensor):
        ...
        for i in range(self.tree_depth):
            reward = self.reward_fun(tensor) 
            tree_result['rewards'].append(reward.view(-1,1))

            tensor = self.tree_transition(tensor)
    ...
    def tree_backup(self, tree_result):
        backup_values = tree_result["values"][-1] #num_actions ^ batchsize x 1
        for i in range(1, self.tree_depth + 1):
            one_step_backup = tree_result['rewards'][-i] + backup_values 
