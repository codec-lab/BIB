...
def train(model, optimizer, train_data, valid_data, num_actions, grad_clip, epochs):
    ...
    return all_gradients[-1], train_losses, valid_losses, training_time

def plot_and_save_all_losses(train_losses_list, valid_losses_list, epochs_list, training_times, titles, filename):
    ...
    for i in range(num_plots):
        final_train_loss = round(train_losses_list[i][-1],1)
        final_valid_loss = round(valid_losses_list[i][-1],1)
        title = (f"{titles[i]}\n"
                 f"Total Training Time: {training_times[i]:.2f} minutes\n"
                 f"Final Train Loss: {final_train_loss:.4f}, Final Valid Loss: {final_valid_loss:.4f}")
        plt.subplot(rows, cols, i + 1)
        plt.plot(range(epochs_list[i]), train_losses_list[i], label='Training Loss')
        plt.plot(range(epochs_list[i]), valid_losses_list[i], label='Validation Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        #plt.title(titles[i] + f'\nTotal Training Time: {training_times[i]:.2f} minutes')
        plt.title(title)
        plt.legend()
        plt.grid(True)
        plt.xticks(range(0, epochs_list[i], 5))
...
def main():
                    ...
                    for gc in grad_clip:
                        for ep in epochs:
                            ...
                            try:
                                ...
                                for i in range(10):
                                    try:
                                        _, train_losses, valid_losses, training_time = train(model= model, optimizer=optimizer, train_data=train_data, valid_data=valid_data, num_actions=na, grad_clip=gc, epochs=ep)
                                        train_losses_list.append(train_losses)
                                        valid_losses_list.append(valid_losses)
                                        epochs_list.append(ep)
                                        training_times.append(training_time)
                                        titles.append(f'NA: {na}, ED: {ed}, DR: {gam}, LR: {lr}, GC: {gc}')
                                        ...
                                        with open('final_losses2.txt', 'a') as f:
                                            ...
                                            f.write(f"Final Validation Loss: {valid_losses[-1]}\n")
                                            f.write("\n\n")
                                        break
                                    except:
                                        raise ValueError("NAN LOSS")
    ...
    plot_and_save_all_losses(train_losses_list, valid_losses_list, epochs_list, training_times, titles, 'all_plots.png')
